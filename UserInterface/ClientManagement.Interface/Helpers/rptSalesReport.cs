...
{
    public class rptSalesReport : GenericExcelReport
    {
        DataSet ds;

        public rptSalesReport(string xmlData)
        ...
        public byte[] GetFile(DateTime EndDate)
        {
            ...
            List<SheetNames> sheets = new List<SheetNames> { SheetNames.Payments, SheetNames.NotPaid };
        ...
        private Worksheet GetNewWorksheet(SheetNames sheetName, ref int row)
        {
            ...
            switch (sheetName)
            {
                case SheetNames.Payments:
                    ...
                    row++;row++; break;             
                case SheetNames.NotPaid:
                    SetHeadingCells(ws.GetCell(row, 3), "Non Paid Report", false);
                    row++;;row++;break;
            }
        ...
        private enum SheetNames { Payments, NotPaid}
